...
void carToCarReceive() {
  static unsigned long lastMillisR = 0;                          //charge timer
  if (car2car) {
    FromCar2Car = false;                                         //cant "send" battery whilst receiving
  }

  if ((strcmp(incomingMsg1, "car2car charge available") == 0)) { //if string equals Msg1
    if (car2car) {                                               //if car2car is true, button 2 on website is pressed
      strcpy(outgoingData.msg1, "accepted car2car charge");
      espNowSend();                                              //sends accepted charge message
      if (millis() - lastMillisR >= chargeInterval) {            //5percent battery every 5 seconds
        lastMillisR = millis();
        if (receivedBattery >= 94.0) {                           //if batterylevel is 94 or above
          strcpy(outgoingData.msg1, "car2car charge stopped");   //sends a car2car stop message
          espNowSend();
          car2car = false;                                       //receive battery is set to false, charging is stopped
        }
        else if ((strcmp(incomingMsg1, "car2carCharge stopped") == 0)) {  //can also be stopped by incoming message from the other party
          car2car = false;
        }
        Wire.beginTransmission(8);                               //slave handles the battery level change with wire communication
        Wire.write('y');  
        Wire.endTransmission();
      }
    }
...
void CarToCarSend() {
  static unsigned long lastMillisS = 0;                           //charge timer

  if (FromCar2Car) {                                              //cant "send" battery whilst receiving
    car2car = false;                                              //sets receiving to false
  }

  if ((receivedBattery >= 50.0) && FromCar2Car) {                 //can only "send" if battery level is above 50percent
    strcpy(outgoingData.msg1, "car2car charge available");        //changes outgoing message
    espNowSend();                                                 
    if ((strcmp(incomingMsg1, "accepted car2car charge") == 0)) { //checks if chosen string matches received message 1
      if (millis() - lastMillisS >= chargeInterval) {
        lastMillisS = millis();
        Wire.beginTransmission(8);
        Wire.write('g');                                          //slave handles decreasing battery level with switch case case "g"
        Wire.endTransmission();
        if ((receivedBattery <= 50.0) || (strcmp(incomingData.msg1, "car2car charge stopped")==0)) { //can be stopped by battery level og incoming "stop" message
          FromCar2Car = false;                                    //stops sending battery
          strcpy(outgoingData.msg1, "car2car charge stopped");
          espNowSend();                                           //sends stop message
        }
      }
    }
  }
